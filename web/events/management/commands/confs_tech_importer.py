...
from events.management.sources import source_urls_javascript
...
class Command(BaseCommand):
    ...
    def import_events(self, source_urls_year):
        ...
        for source_url in source_urls[source_urls_year]:
            ...
            for event_json_data in input_json_data:
                ...
                if not Event.objects.filter(title=event_name).exists():
                    ...
                    msg = "Event {:d} - {:s} created successfully".format(
                        event_obj.id, event_obj.title)
                    print(msg)
                else:
                    event_obj = event.update_event(event_json_data)
                    event_obj.save()
                    msg = "Event {:d} - {:s} updated successfully".format(
                        event_obj.id, event_obj.title)
                    print(msg)

                # create or update Event Edition
                event_edition_name = "{:s} {:s}".format(event_name,
                                                        source_urls_year)

                if not Edition.objects.filter(
                        title=event_edition_name).exists():
                    event_edition_obj = edition.create_event_edition(
                        event_json_data, source_urls_year, event_obj)
                    msg = "Event Edition {:d} - {:s} created successfully".format(
                        event_edition_obj.id, event_edition_obj.title)
                    print(msg)
                else:
                    event_edition_obj = edition.update_event_edition(
                        event_json_data, source_urls_year, event_obj)
                    event_edition_obj.save()
                    msg = "Event Edition {:d} - {:s} updated successfully".format(
                        event_edition_obj.id, event_edition_obj.title)
                    print(msg)
